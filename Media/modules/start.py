...
async def create_saweria(client, cb):
    ...
    try:
        ...
        url = f"https://pg-saweria.autsc.My.id/api/create?username={user.get('username')}&amount={amount}&email={user.get('email')}"
...
async def check_saweria(client, callback_query):
    ...
    try:
        url = f"http://pg-saweria.autsc.my.id/check-payment?id={pay_invoice}"
